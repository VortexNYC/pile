...
export class WorkspaceDO extends DurableObject<AppEnv> {
  ...
  // ---- agent skills ----
  createAgentSkill(input: Omit<data.AgentSkillInput, "organizationId" | "createdById"> & { createdById?: string }) {
    return data.createAgentSkill(this.db, this.organizationId, {
      ...input,
    });
  }

  getAgentSkill(skillId: string) {
    return data.getAgentSkill(this.db, this.organizationId, skillId);
  }

  listAgentSkills() {
    return data.listAgentSkills(this.db, this.organizationId);
  }

  updateAgentSkill(skillId: string, input: Partial<data.AgentSkillInput>) {
    return data.updateAgentSkill(this.db, this.organizationId, skillId, input);
  }

  deleteAgentSkill(skillId: string) {
    return data.deleteAgentSkill(this.db, this.organizationId, skillId);
  }

  async invokeAgentSkill(skillId: string, input: Record<string, unknown>) {
    const skill = await data.getAgentSkill(this.db, this.organizationId, skillId);
    if (!skill) return { ok: false, status: undefined, body: null, error: "Skill not found" };
    if (!skill.enabled) return { ok: false, status: undefined, body: null, error: "Skill disabled" };
    let invoke: Record<string, unknown>;
    try {
      invoke = JSON.parse(skill.invoke);
    } catch {
      return { ok: false, status: undefined, body: null, error: "Invalid invoke config" };
    }
    if (invoke.type === "http") {
      const url = String(invoke.url ?? "");
      const method = String(invoke.method ?? "POST");
      const headers = (invoke.headers as Record<string, string>) ?? { "Content-Type": "application/json" };
    ...
    return { ok: false, status: undefined, body: null, error: "Unsupported invoke type" };
  }

  // ---- agent conversations ----
  createAgentConversation(input: data.AgentConversationInput) {
    return data.createAgentConversation(this.db, this.organizationId, input);
  }

  getAgentConversation(conversationId: string) {
    return data.getAgentConversation(this.db, this.organizationId, conversationId);
  }

  listAgentConversations(context?: { contextType?: "issue" | "document" | "project" | "workspace"; contextId?: string }) {
    return data.listAgentConversations(this.db, this.organizationId, context);
  }

  updateAgentConversation(
    conversationId: string,
    input: Partial<data.AgentConversationInput> & { status?: "open" | "closed" }
  ) {
    return data.updateAgentConversation(this.db, this.organizationId, conversationId, input);
  }

  deleteAgentConversation(conversationId: string) {
    return data.deleteAgentConversation(this.db, this.organizationId, conversationId);
  }

  createAgentMessage(input: data.AgentMessageInput) {
    return data.createAgentMessage(this.db, input);
  }

  listAgentMessages(conversationId: string) {
    return data.listAgentMessages(this.db, conversationId);
  }
