...
const PYTHON_RUNNER = [
  ...
  "        cmd, cwd=REPO_DIR, env=cli_env(), capture_output=True, text=True, timeout=7200",
  "    )",
  "    print('cursor-agent exit:', result.returncode)",
  "    if result.stdout:",
  "        print(result.stdout[-4000:])",
  "    if result.stderr:",
  "        print(result.stderr[-2000:], file=sys.stderr)",
  "    if result.returncode != 0:",
  "        raise RuntimeError(f'cursor-agent -p failed ({result.returncode}): {result.stderr[-500:]}')",
