...
const PYTHON_RUNNER = [
  ...
  "    result_text = json.dumps({'output_tail': output})",
...
export class DevinCliAgentProvider implements AgentProvider {
    ...
    return (
      list.items.find(
        (s) =>
          ...
          s.name ===
            `vortex-devin-${sessionId.replace(/[^a-zA-Z0-9]/g, "").slice(0, 12)}`
      ) ?? null
    );
  }
  ...
  private async readResult(
    base: string,
    apiKey: string
  ): Promise<z.infer<typeof devinResultSchema> | null> {
    const res = await fetch(`${base}/process/execute`, {
      ...
      body: JSON.stringify({
        command: "cat /tmp/devin-result.json",
        cwd: "/",
      }),
    });
    if (!res.ok) return null;
    ...
    const parsed = devinResultSchema.safeParse(JSON.parse(data.result));
  ...
  private async start(
    ...
    const name = `vortex-devin-${shortId}`;
