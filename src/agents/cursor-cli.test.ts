...
describe("CursorCliAgentProvider", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  it("throws when credentials are missing", async () => {
    const provider = new CursorCliAgentProvider(
      cliEnv({ CURSOR_API_KEY: undefined, AGENT_PROVIDER_TOKEN: undefined })
    );
    await expect(
      provider.dispatch("org-1", issueFixture(), "auto", {
        sessionId: "sess-1",
        gitIdentity: gitIdentityFixture(),
      })
    ).rejects.toThrow("CURSOR_API_KEY is not configured");
  });
  ...
  it("dispatches and starts a Daytona sandbox", async () => {
    ...
    expect(result.agentId).toBe("cursor-cli");
    ...
    expect(body.env.MODEL).toBe("auto");
    expect(body.env.REPO).toBe("VortexNYC/pile");
    expect(body.env.CURSOR_API_KEY).toBeDefined();
