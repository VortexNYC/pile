...
Install the `@vortex-api/pile` package and authenticate with an API key. The CLI defaults to `http://127.0.0.1:8787` (local `wrangler dev`); point it at your deployment with `PILE_BASE_URL` or `pile config set --base-url`.
...
The `@vortex-api/pile` package is an `openapi-fetch` client typed against the Pile OpenAPI spec.
