...
Vortex currently exposes **85 OpenAPI paths / 132 MCP tools**. Linear's public GraphQL surface has **626 root operations** (170 + 374 + 82), so Vortex covers the high-frequency subset but not the long tail.

## Biggest blockers to "never touch Linear again"

1. **Zendesk/GitLab/Intercom integrations** — GitHub and Slack are wired.
2. **Documents, customers, releases, audit log** — entirely unmodeled domains (see gap map).
3. **Notification delivery** — in-app only; no email/push or per-user delivery preferences.
