...
`wrangler.toml` is git-ignored; the canonical template is `wrangler.toml.example`. Copy it to `wrangler.toml` and fill in your own values:
...
`selfhost` prints the remaining steps: set `BETTER_AUTH_URL`, `ALLOWED_ORIGINS`, and `SLACK_REDIRECT_URI` in `wrangler.toml` `[vars]` to your Worker URL, then `wrangler secret put BETTER_AUTH_SECRET` (plus optional `DEVIN_TOKEN`, `GITHUB_WEBHOOK_SECRET`, `SLACK_*`).

### Manual

1. `pnpm install`
2. Copy `wrangler.toml.example` to `wrangler.toml` and edit `[vars]`:
   - `BETTER_AUTH_URL`, `ALLOWED_ORIGINS`, and `SLACK_REDIRECT_URI` to your Worker URL.
   - `D1` database name, `ATTACHMENTS_BUCKET` name, and `WEBHOOK_QUEUE` name in their respective binding blocks.
3. Add secrets:
   ```bash
   ...
   ```
4. Apply D1 migrations and deploy:
   ```bash
   wrangler d1 migrations apply D1 --remote
   wrangler deploy
   ```

`pnpm deploy` targets the hosted `production` environment and is not for self-hosting.
